...
//		return !weekday || vacation;
    ...
    /* RESUME HERE. Thursday pep talk: 1 - 18 was hard, but you got this!
     ...
     */
    public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) { //what a doozie 
    	if(mealAmount > 10 && mealAmount <= 15 && !isBirthday) {
    		return "special";
    	}
    	else if(mealAmount > 5 && mealAmount <= 10  && isBirthday) {
    		return "special";
    	} 
    	else if(mealAmount > 15 && !isBirthday) {
    		return "ginormous";
    	} 
    	else if(mealAmount > 10 && isBirthday) {
    		return "ginormous";
    	}
    	else {
    		return "standard";
	...
	public int sortaSum(int a, int b) {
		int sum = a + b; // need to return a sum, so initialize a value to sum
		if(sum < 10 && sum > 19) {
			return sum; // exclusive to 10-19, return the sum
		} else if(sum >= 10 && sum <= 19) {
			return 20; //inclusive to 10-19, return 20
		} else {
			return sum; //any other scenario outside of 10-19 inclusive, sum (may not need this line?)
		}
	}
	...
	public String alarmClock(int day, boolean vacation) {
		if(vacation) {
			if(day > 0 && day < 6) { 
				return "10:00"; //weekday alarm on vacation
			} else {
				return "off"; //this should take care of the weekend vacation alarm clock?
			}
		}
		if(day > 0 && day < 6) { //weekday non-vacation alarm clock
			return "7:00";
		} else {
			return "10:00"; // if not in this range, then it's a weekend, so 10.
		}
	}
	...
	public boolean in1To10(int n, boolean outsideMode) {
		if(outsideMode == true && (n <= 1 || n >= 10)) { // outside is true n less than 1 or more than 10
			return true;
		} else if(!outsideMode && n >= 1 && n <= 10) { // outside false, inclusive 1-10
			return true;
		} else 
		return false;
	}
	 ...
	 */
	public boolean specialEleven(int n) { // needs an or statement and %
		if(n % 11 == 0 || n % 11 == 1) {
		return true;
		} else {
			return false;
	...
	/*
	 ...
	 more20(22) → true
	 */
	public boolean more20(int n) { // exact same as above
		if(n % 20 == 1 || n % 20 == 2) {
		return true;
		} else {
			return false;
	...
	/*
	 ...
	 old35(15) → false
	 */
	public boolean old35(int n) { // one OR the other... think, are these different? same as above, but EXCLSUIVE
		if(n % 3 == 0 ^ n % 5 == 0) {
			return true;
		} else {
		return false;
	...
	public boolean less20(int n) {
		if(n % 20 == 19 || n % 20 == 18) { //  think: if n % 20 equals 1 or 2 less than 20 respectively
		return true;
		} else {
			return false;
		}
	...
	/*
	 ...
	 nearTen(19) → true
	 */
	public boolean nearTen(int num) { // return true if num % 10 is +2 or less OR 10 less than the same.
	...
	public int teenSum(int a, int b) {
		int sum = a + b; // use an int to return the sum of a + b
		if(a >= 13 && a <= 19) { // if a is teen, lucky!
			return 19;
		} else if(b >= 13 && b <= 19) { // if b is teen, lucky!
			return 19;
		} else {
			return sum; // all else, unlucky/not a teen
		}
	}
	...
	public boolean answerCell(boolean isMorning, boolean isMom, boolean isAsleep) {
		if(isAsleep) {
			return false; // can't answer if you're asleep
		} else {
			if(isMorning) {
			if(isMom) {
				return true; //only answer for mom in the morning
		} else {
			return false; //never unless it's mom
		}
		} else { 
			return true; //any other time, true
		}
	}
	...
	public int teaParty(int tea, int candy) {
		if(tea < 5 || candy < 5) { //if either one is less than 5, automatically bad party
			return 0;
		} else { 
			if((tea * 2) <= candy || (candy * 2) <= tea) { // if one is twice that of the other, great party
				return 2;
			}
		} 
			return 1; //any other scenario is good
	}
	...
	public boolean twoAsOne(int a, int b, int c) {
		if((a + b == c) || (a + c == b) || (b + c == a)) //only these statements will satisfy true
		return true;
			return false;
	}
	 ...
	 */
	public boolean inOrder(int a, int b, int c, boolean bOk) { // if bOk, only c > b. if c > b > a = true, and no bOK, true. else false.
		if(bOk) {
			if(c > b) 
				return true;
			else
	 ...
	 */
	public boolean inOrderEqual(int a, int b, int c, boolean equalOk) { //if equal
		if(equalOk && a <= b && b <= c) {
			return true;
		}
		if(!equalOk && a < b && b < c) {
	...
	/*
	 ...
	 loneSum(3, 3, 3) → 0
	 */
	public int loneSum(int a, int b, int c) { //if all three are same, return 0
		int sum = a + b + c;
		if(a == b && b == c)
			return 0;
		else { // if two are the same, return the third.
			if(a == b)
				return c;
			if(a == c)
				return b;
		...
		return sum; // everything else gets summed out
		}
